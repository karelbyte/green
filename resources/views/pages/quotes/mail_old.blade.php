...
<head>
    ...
    <title>GreenCenter - Cotización: {{$data['data']->id}}</title>
    <style>
        div {
            display: block;
            float: left;
...
<div>
    <p>Estimado: <strong>{{$data['data']['globals']['client']['name']}}</strong> nos complace hacerle llegar su cotización.</p>

    <p>Cotizacion No: <strong>{{$data['data']->id}}</strong>

    <p>Numero de confirmación: <strong>{{$data['data']['token']}}</strong></p>
